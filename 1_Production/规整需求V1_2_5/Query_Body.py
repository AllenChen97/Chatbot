...
class Query_Body(Demand_Interpret, Query_Record): #机器人 和 bug-handler
    def __init__(self, dic):
        ...
        self.version = "1.2.5"  # 上线前需要更改
        self.is_production = dic.get('is_production')
        self.subinterface = dic.get('subinterface')   # !! 0-模糊  1-规整  2-明细
        self.is_data = 1
        self.is_report = 0
    ...
    def query(self,sql):
        conn2 = ck.connect(host='172.16.92.170', port='19000', database='default', user='cp_chenjinzhao1',
                           password='chenjinzhao1&%#7175')
        cur = conn2.cursor()
        cur.execute(sql)
    ...
    def run(self):
        ...
        ########################################################### 一、解析
        try:
            self.elmDistinguisher()             # ① 部门、时间、指标、粒度 元素识别
        except:
            self.is_error = 1
            self.error_type = 0
        ...
        try:
            self.indexMatch()                   # ⑤将'指标'分词后与set逐个匹配
            if self.indcode == "":
                self.is_error = 1
                self.error_type = 3
