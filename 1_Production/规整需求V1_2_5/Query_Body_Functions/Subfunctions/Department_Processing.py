...
class Department_Processing:
    ...
    def ctypeCompare(self):
        ...
        if dept_level is None:
            self.ctype_cp = -1
        elif dept_level.find('事') >= 0 or dept_level.find('营') >= 0:
            self.ctype_cp = 2
            ...
            self.ctype_cp = 5
        elif dept_level.find('人') >= 0 or dept_level.find('顾问') >= 0 or dept_level.find('员工') >= 0:
            self.ctype_cp = 6
        else:
            self.ctype_cp = None   # 如果有部门粒度但解析失败，就故意让dateS_G_O的比较报错
